...
Процедура UploadCatalogНаСервере()
	...
	if CatalogItem.IsFolder then
		...
		SabatexExchange.RegisterMessageForNode(nodeName,,CatalogItem.GetObject(),true);
	endif;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	nodes = GetDestinationNodes();
	
	for each node in nodes do
		Элементы.NodeName.СписокВыбора.Добавить(node.NodeName);	
	    Элементы.UploadObjectFilterNodeName.СписокВыбора.Добавить(node.NodeName);
    enddo; 
...
Процедура RestoreStateAtServer()
	...
	state =  SabatexJSON.Deserialize(Sabatex.ValueOrDefault(reg.Value,"{}"));
	...
	for each item in  of do
		...
		row.Checked = Sabatex.ValueOrDefault(item["Checked"],false);
		...
		row.Касса = Справочники.Кассы.ПолучитьСсылку(new UUid(?(item["Касса"] = undefined,Sabatex.GetEmptyUUID(),item["Касса"])));
	enddo;	
    endif;
...
Процедура SaveStateНаСервере()
	...
	reg.Value  = SabatexJSON.Serialize(state);
	reg.Write(true);
КонецПроцедуры
